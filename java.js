function library(){
    ...
    const editBookForm = (i) => {
        newBookDialog.showModal()
        inputOne.value = myLibrary[i].title
        inputTwo.value = myLibrary[i].author
        inputThree.value = myLibrary[i].pages
        inputFour.checked = myLibrary[i].read
        inputFive.value = myLibrary[i].url
        newBookSubmBtn.id = i
    }

    const editBookInLibrary = () =>{
        let book, i, title, author, pages, read, url
        i = newBookSubmBtn.id
        title = inputOne.value
        author = inputTwo.value
        pages = inputThree.value
        read = inputFour.checked
        url = inputFive.value
    ...
    const displayAllBooks= () => {
        ...
        button.addEventListener('click', ()=>{
            display.replaceChildren()
            const x = library.splice(button.id, 1);
            displayAllBooks() 
        })
